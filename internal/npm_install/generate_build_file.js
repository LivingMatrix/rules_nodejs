...
const whitelistedExtensions = args[0] ? args[0].split(',') : [];
...
function generateRootBuildFile(pkgs) {
  const srcs = pkgs.filter(pkg => !pkg._isNested)
                   .map(pkg => `"//node_modules/${pkg._dir}:${pkg._name}__files",`)
                   .join('\n        ');
  const binFiles =
      listFiles('node_modules/.bin').map(f => `"node_modules/.bin/${f}",`).join('\n        ');
...
function listFiles(rootDir, subDir = '') {
  ...
  return fs.readdirSync(dir).reduce(
      (files, file) => fs.statSync(path.join(dir, file)).isDirectory() ?
          files.concat(listFiles(rootDir, path.join(subDir, file))) :
          files.concat(path.join(subDir, file)),
      []);
}
...
function filterFilesForFilegroup(files, exts = []) {
  files = files
              // Bazel target names must be composed entirely of characters drawn
              ...
              .filter(f => path.basename(f) !== 'WORKSPACE');
  if (exts.length) {
...
function printPackage(pkg) {
  const sources = filterFilesForFilegroup(pkg._files, whitelistedExtensions);
